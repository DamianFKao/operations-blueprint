...
Decompose your part to its atoms (every material, labor step, unit of time and overhead) and model it as related tables, not a pile of documents. Once the product is honest, joinable data, pricing, quoting, scheduling, and the customer-facing side all become functions of that data.
...
Because your parts are configured or custom, a quote line is not always a catalog item: it can be an assembly built for that job from its own materials and operations. Let a quote line reference either a defined product or a per-quote assembly, and price both the same way through the engine.
